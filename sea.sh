#! /bin/bash
nodejs_version="${NODEJS_VERSION:-20.15.1}"
NODE_DIR=".sea/node"

mkdir -p "${NODE_DIR}"
npx esbuild main.ts --bundle --platform=node --target=node20.15.1 --outfile=.sea/main.js
...
do
    ...
    cp "${NODE_DIR}/node-v${nodejs_version}-${os}/bin/node" ".sea/jemena-mqtt-${os}"
    rm -rf "${NODE_DIR}/node-v${nodejs_version}-${os}"
    npx postject ".sea/jemena-mqtt-${os}" NODE_SEA_BLOB .sea/prep.blob --sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2
done
